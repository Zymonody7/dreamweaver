...
import {
  ...
  X, Moon, Sun, Star, Brain, ChevronLeft, Download, Home, Sparkles, Activity, Globe, Users, Edit, Save, Trash2
} from 'lucide-react';
...
const UniverseView = ({ dreams }: { dreams: Dream[] }) => {
  ...
  // Load public dreams from backend
  useEffect(() => {
    const loadPublicDreams = async () => {
      setLoadingPublic(true);
      try {
        const response = await fetch('/api/dreams/public?limit=10');
        if (response.ok) {
  ...
  const handleWeave = async () => {
    ...
    try {
      ...
      const collectiveSnippets = publicDreams.slice(0, 10).map(d => `Collective Echo: ${d.content}`);
           ...
           {!story ? (
             <div className="space-y-8">
               ...
               <div>
                 ...
                 {loadingPublic ? (
                   <div className="text-slate-500 text-sm">Loading collective dreams...</div>
                 ) : publicDreams.length > 0 ? (
                   <div className="text-left bg-black/20 p-4 rounded-xl border border-white/5 mb-4 text-xs text-slate-500 font-mono">
                      <p className="mb-2 uppercase tracking-widest font-bold text-slate-400">Detected Signals ({publicDreams.length}):</p>
                      <ul className="list-disc pl-4 space-y-1">
                        {publicDreams.slice(0, 3).map((dream, i) => (
                          <li key={i} className="truncate">"{dream.content.substring(0, 80)}..."</li>
                        ))}
                        {publicDreams.length > 3 && (
                          <li className="italic opacity-50">...and {publicDreams.length - 3} more signals</li>
                        )}
                      </ul>
                   </div>
                 ) : (
                   <div className="text-slate-500 text-sm mb-4">No public dreams available yet. Be the first to share!</div>
                 )}
               </div>
               <button
                onClick={handleWeave}
                disabled={loading || dreams.length < 1 || publicDreams.length === 0}
    ...
    return (
      <div className="flex flex-col items-center justify-center h-[70vh] text-center p-6">
        <div className="w-32 h-32 bg-night-800 rounded-full flex items-center justify-center mb-6">
          <Users size={48} className="text-mystic-400" />
        </div>
        <h2 className="text-2xl font-serif text-white mb-2">No Public Dreams Yet</h2>
        <p className="text-slate-400 max-w-xs">Be the first to share your dreams with the community!</p>
      </div>
    );
  }
    ...
    <div className="space-y-6 pb-24 md:pb-0 animate-fade-in">
      <header className="flex justify-between items-end mb-6 pt-4 px-2">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">{publicDreams.length} shared dreams from the collective</p>
        </div>
      </header>
            ...
            <div className="flex flex-col">
              ...
              {dream.imageUrl && (
                <div className="w-full h-48 relative">
                  <img src={dream.imageUrl} className="absolute inset-0 w-full h-full object-cover" alt="dream" />
                  <div className="absolute inset-0 bg-black/20 group-hover:bg-transparent transition-colors"></div>
                </div>
              )}
              ...
              <div className="p-4 md:p-6">
                <div className="flex justify-between items-start mb-2">
                  ...
                  <div className="flex gap-1">
                    {[...Array(dream.clarity || 3)].map((_, i) => <Star key={i} size={10} className="text-yellow-500 fill-current" />)}
                  </div>
                </div>

                <h3 className="text-white font-serif font-bold text-lg mb-2 line-clamp-1">
                  {dream.themes?.[0] || 'Untitled Dream'}
                </h3>
        ...
        <div className="p-6 overflow-y-auto flex-1 space-y-6">
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center justify-between bg-night-950/30 p-4 rounded-xl border border-white/5">
              <div>
                <label className="text-sm font-bold text-slate-500 uppercase block mb-1">Share to Universe</label>
                <p className="text-xs text-slate-600">Make this dream visible to the collective</p>
              </div>
              <label className="flex items-center gap-3 cursor-pointer">
                <div className={`w-12 h-6 rounded-full p-1 transition-colors ${isPublic ? 'bg-accent-cyan' : 'bg-night-800'}`}>
                ...
                <span className="text-sm text-slate-400">{isPublic ? 'Public' : 'Private'}</span>
              </label>
            </div>
          </div>
        </div>
...
const DreamDetail = ({ dream, onClose, onUpdate }: { dream: Dream, onClose: () => void, onUpdate: (d: Dream) => void }) => {
  ...
  const [editedEmotionalAnalysis, setEditedEmotionalAnalysis] = useState(dream.analysis?.emotionalAnalysis || '');
  const [editedCreativeStory, setEditedCreativeStory] = useState(dream.analysis?.creativeStory || '');
      ...
      <div className="absolute top-0 left-0 right-0 h-[40vh] md:h-[50vh] overflow-hidden">
        ...
        <div className="absolute top-0 left-0 right-0 p-4 md:p-6 flex justify-between items-center z-20">
          ...
          <div className="flex gap-3">
             <button onClick={exportDream} className="w-10 h-10 rounded-full bg-black/20 backdrop-blur-md flex items-center justify-center text-white hover:bg-white/10 border border-white/10" title="Export">
                <Download size={20} />
             </button>
             <button
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-12">
            <div className="md:col-span-2 space-y-8">
              ...
              <div className="bg-night-900/50 p-6 rounded-2xl border border-white/5">
                <div className="flex justify-between items-center mb-4">
                  <h3 className="text-xs font-bold text-slate-500 uppercase tracking-widest">AI Creative Story</h3>
                  {!isEditingAnalysis ? (
                    <button
              ...
              <div className="bg-night-900/50 p-6 rounded-2xl border border-white/5">
                <div className="flex justify-between items-center mb-4">
                  <h3 className="text-xs font-bold text-slate-500 uppercase tracking-widest">Original Entry</h3>
                  {!isEditingContent ? (
                    <button
            ...
            <div className="space-y-6">
               ...
               <div className="bg-night-900/50 p-6 rounded-2xl border border-white/5">
                 <h3 className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-4">Visibility</h3>
                 <label className="flex items-center justify-between cursor-pointer group">
                   <div className="flex items-center gap-3">
                     <Globe size={16} className={dream.isPublic ? 'text-accent-cyan' : 'text-slate-500'} />
                     <div>
                       <p className="text-sm text-white font-medium">{dream.isPublic ? 'Public' : 'Private'}</p>
                       <p className="text-xs text-slate-500">{dream.isPublic ? 'Visible in Community' : 'Only you can see this'}</p>
                     </div>
                   </div>
                   <div
                   ...
                   >
                     <div className={`w-4 h-4 bg-white rounded-full shadow-md transform transition-transform ${dream.isPublic ? 'translate-x-6' : 'translate-x-0'}`}></div>
                   </div>
                 </label>
               </div>
...
function App() {
  ...
  const handleSaveNewDream = async (data: any) => {
    try {
      ...
      const imageUrl = await DoubaoService.generateDreamImage(data.content, analysis.themes);
    ...
    <div className="min-h-screen text-slate-200">
      ...
      <main className="md:pl-64 min-h-screen">
        <div className="max-w-5xl mx-auto p-4 md:p-8">
          ...
          {activeTab === 'community' && <CommunityView onSelect={setSelectedDream} />}
